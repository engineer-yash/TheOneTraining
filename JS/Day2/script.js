...
*/
let userInputNumber = prompt("Enter Multiple Numbers");

let output = "";
for (let i = 0; i < userInputNumber.length; i++) {
  if (!isNaN(userInputNumber[i]) && userInputNumber[i] % 2 === 0 && i !== userInputNumber.length - 1) {//if the number is even and not the last number
    output = output + userInputNumber[i] + "-";
  } else if (!isNaN(userInputNumber[i])) {
    output = output + userInputNumber[i];
  }
}
console.log(output);
